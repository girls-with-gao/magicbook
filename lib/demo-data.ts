...
const demoAnalyses: DrawingAnalysis[] = [
  demoAnalysis,
  {
...
function chapterIndex(chapter: number) {
  return Math.min(MAX_CHAPTERS, Math.max(1, Math.round(chapter) || 1)) - 1;
}

export function demoAnalysisFor(chapter = 1): DrawingAnalysis {
